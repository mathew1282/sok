...
function renderPatrole() {
    ...
    funkcjonariusze.forEach(osoba => {
        ...
        const originalIndex =
            appState.dane.rows.indexOf(osoba);

        const selected =
            selectedPatrolMembers.includes(
                originalIndex
            )
            ? "selected"
            : "";
...
// =====================================
// WYBÓR CZŁONKA PATROLU
// =====================================

function togglePatrolPerson(index){

    const pos =
        selectedPatrolMembers.indexOf(
            index
        );

    if(pos > -1){

        selectedPatrolMembers.splice(
            pos,
            1
        );
...
function getPersonName(osoba){
    ...
    return `${osoba["Stopień"] || ""}
${osoba["Nazwisko"] || ""}
${osoba["Imię"] || ""}`.trim();
}

// =====================================
// LISTY DOWÓDCA / KIEROWCA
// =====================================
...
function createPatrol() {
    ...
    if(
        selectedPatrolMembers.length === 0
    ){

        alert(
            "Wybierz funkcjonariuszy"
        );
